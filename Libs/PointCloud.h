...
#pragma omp parallel for
        for (int v = 1; v < height - 1; ++v) {
            for (int u = 1; u < width - 1; ++u) {
                ...
                //       Store the result in normalsTmp[idx].
                Vector3f hor = Vector3f(1.0, 0, du);
                Vector3f ver = Vector3f(0, 1.0, dv);
                normalsTmp[idx] = ver.cross(hor); // Needs to be replaced.
                normalsTmp[idx].normalize();
                normalsTmp[idx] = Vector3f(du, dv, 1).normalized();
            }
        }
