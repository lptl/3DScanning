...
public:
    ...
    T* getData() const {
        // m_array is the angle-axis rotation + translation
        return m_array;
    }
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // To apply increment (pose parameters) to the source point, you can use the PoseIncrement
        // class.
        // Important: Ceres automatically squares the cost function
        // (i.e. the cost function is already squared).
        // Important: The weight of the constraint is already applied in the cost function.
        // Important: The memory for the residual vector needs to be reserved (i.e. on the stack)
        PoseIncrement<T> poseIncrement(const_cast<T*>(pose));
        T targetPoint[3], sourcePoint[3];
        sourcePoint[0] = T(m_sourcePoint[0]); sourcePoint[1] = T(m_sourcePoint[1]); sourcePoint[2] = T(m_sourcePoint[2]);
        poseIncrement.apply(sourcePoint, targetPoint);
        residuals[0] = T(LAMBDA) * T(m_weight) * (targetPoint[0] - T(m_targetPoint[0]));
        residuals[1] = T(LAMBDA) * T(m_weight) * (targetPoint[1] - T(m_targetPoint[1]));
        residuals[2] = T(LAMBDA) * T(m_weight) * (targetPoint[2] - T(m_targetPoint[2]));
        return true;
    }
...
protected:
    ...
    const float LAMBDA = 0.1f; // original 0.1f
};
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        poseIncrement.apply(sourcePoint, targetPoint);
        T intermediate[3];
        intermediate[0] = targetPoint[0] - T(m_targetPoint[0]);
        intermediate[1] = targetPoint[1] - T(m_targetPoint[1]);
        intermediate[2] = targetPoint[2] - T(m_targetPoint[2]);
        residuals[0] = T(LAMBDA) * T(m_weight) * (intermediate[0] * T(m_targetNormal[0]) + intermediate[1] * T(m_targetNormal[1]) + intermediate[2] * T(m_targetNormal[2]));
        return true;
    }
...
protected:
    ...
    const float m_weight;
    const float LAMBDA = 1.0f; // original 1.0f
};
...
protected:
    ...
    std::vector<Vector3f> transformPoints(const std::vector<Vector3f>& sourcePoints, const Matrix4f& pose) {
        ...
        std::cout << "finished transformation of points" << std::endl;
    ...
    std::vector<Vector3f> transformNormals(const std::vector<Vector3f>& sourceNormals, const Matrix4f& pose) {
        ...
        std::cout << "finished transformation of normals" << std::endl;
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                // degrees.
                auto dot_product = sourceNormal[0] * targetNormal[0] + sourceNormal[1] * targetNormal[1] + sourceNormal[2] * targetNormal[2];
...
public:
    ...
    virtual void estimatePose(const PointCloud& source, const PointCloud& target, Matrix4f& initialPose) override {
        ...
        for (int i = 0; i < m_nIterations; ++i) {
            ...
            std::cout << "starting nearest neighbor search" << std::endl;
    ...
    void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
        ...
        for (unsigned i = 0; i < nPoints; ++i) {
            ...
            if (match.idx >= 0) {
                ...
                // problem.AddResidualBlock(new ceres::AutoDiffCostFunction<PointToPointConstraint, 3, 6>(new PointToPointConstraint(sourcePoint, targetPoint, match.weight)),
                //                          nullptr, poseIncrement.getData());
                problem.AddResidualBlock(PointToPointConstraint::create(sourcePoint, targetPoint, match.weight), nullptr, poseIncrement.getData());
...
public:
    ...
    virtual void estimatePose(const PointCloud& source, const PointCloud& target, Matrix4f& initialPose) override {
        ...
        for (int i = 0; i < m_nIterations; ++i) {
            ...
            auto transformedPoints = transformPoints(source.getPoints(), estimatedPose);
            auto transformedNormals = transformNormals(source.getNormals(), estimatedPose);
            std::cout << "now starting nearest neighbor search" << std::endl;
            auto matches = m_nearestNeighborSearch->queryMatches(transformedPoints);
            pruneCorrespondences(transformedNormals, target.getNormals(), matches);
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            A.block<1, 3>(i * 4, 0) = Vector3f(n[2] * s[1] - n[1] * s[2], n[0] * s[2] - n[2] * s[0], n[1] * s[0] - n[0] * s[1]);
            A.block<1, 3>(i * 4, 3) = n;
            b[i * 4] = n.transpose() * (d - s);

            // TODO: Add the point-to-point constraints to the system
            A.block<1, 3>(i * 4 + 1, 0) = Vector3f(0, s[2], -s[1]);
            A.block<1, 3>(i * 4 + 1, 0) = Vector3f(1, 0, 0);
            A.block<1, 3>(i * 4 + 2, 0) = Vector3f(-s[2], 0, s[0]);
            A.block<1, 3>(i * 4 + 2, 0) = Vector3f(0, 1, 0);
            A.block<1, 3>(i * 4 + 3, 0) = Vector3f(s[1], -s[0], 0);
            A.block<1, 3>(i * 4 + 3, 0) = Vector3f(0, 0, 1);
            b[i * 4 + 1] = d[0] - s[0];
            b[i * 4 + 2] = d[1] - s[1];
            b[i * 4 + 3] = d[2] - s[2];
        ...
        std::cout << "trying to solve the problem, matrix size " << 4 * nPoints << " x 6 " << std::endl;
        ...
        MatrixXf intermediate(6, 4 * nPoints);
        intermediate.setZero();
        intermediate.block<6, 6>(0, 0) = svd.matrixV() * S;
        x = intermediate * svd.matrixU().transpose() * b;
