...
#define DENSE_MATCHING_METHOD "qd" // bm, sgbm, qd
...
void compute_disparity_map(cv::Mat left, cv::Mat right, cv::Mat& disp) {
    ...
    if (compare_string(DENSE_MATCHING_METHOD, "bm")) {
        ...
        if (USE_POST_FILTERING) {
            ...
            wls_filter->filter(disp, left, disp, right_disp);
        }
    }
        ...
        cv::Ptr<cv::stereo::QuasiDenseStereo> left_matcher = cv::stereo::QuasiDenseStereo::create(left_gray.size());

        left_matcher->process(left_gray, right_gray);
